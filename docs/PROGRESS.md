...
- Consume the exact vendored `@ayenisholah/perpeto-api-client@0.15.0` tarball; the lossless-decimal fields introduced in `0.14.0` remain strings across the API boundary, and the matching backend client passes local typecheck and 21 contract tests.
...
- Primary backend connector WebSocket/sequence-recovery code is implemented locally, but its linked Rust tests and credentialed CEX sandbox/demo/shadow evidence are incomplete, so M3 and production trading remain unavailable.
...
- GitHub Shared Document Drift run `29811491233` is red on remote `main` `9f9d58c` because `docs/DECISION.md` had not yet received the backend update. The current local shared-document set is byte-for-byte aligned across both repositories but is not yet remote-verified.
- API client `0.15.0` is vendored but unpublished; `0.7.0` is the latest registry release confirmed by workflow evidence.
- M4 personal-tenant status and the backend's first forced-RLS/private-runtime slice are implemented locally but not database-verified. Native passkeys, ephemeral exchange-credential enrollment, account preflight/live eligibility and pilot controls are not implemented. The phone will authorize changes but will not store or decrypt enrolled credentials.
...
- Permanent consumption of client `0.15.0` depends on publishing that version and replacing the checked-in tarball through the versioned API-update workflow.
...
2. After the backend passes linked local/approved-environment tests and M3 evidence is available, verify the Exchanges monitoring path on a real device.
3. Run the lossless-decimal mobile contract on a real device after backend CI verifies the shared revision.
4. Add cross-account/cache/logout and physical-device evidence for the implemented personal-tenant session slice; do not add credential fields before the one-use, passkey-authorized Vault enrollment contract exists.
