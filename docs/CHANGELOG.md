...
- Realigned 15 dependencies with the versions Expo SDK 57 expects, which `npx expo-doctor` had been failing on. The drift was invisible because CI stops at the first failure and `npm audit` failed earlier in the same job, so the check had not actually run since the packages fell behind. Fourteen are patch bumps within SDK 57 — `expo` 57.0.7→57.0.9, `expo-updates` 57.0.8→57.0.11, `expo-router` 57.0.7→57.0.9, `react-native` 0.86.0→0.86.2, `react-native-reanimated` 4.5.0→4.5.1, `react-native-worklets` 0.10.0→0.10.1 and the remaining `expo-*` packages — plus one minor, `react-native-screens` 4.25.2→`~4.26.0`, which is the version SDK 57 pins. Running versions the SDK does not expect is itself the defect, so these were fixed rather than suppressed through `expo.install.exclude`. Verified: `expo-doctor` passes 20/20, `npm ci` installs the lock cleanly, and lint, typecheck and all 15 tests pass; an iOS bundle exports without error (2.6 MB Hermes bytecode), which exercises module resolution and the Metro graph but not the native build.
...
- Made the web target usable for local inspection, because the published Expo Go cannot run this SDK on older iOS. `expo-secure-store` has no web implementation — its web entry point is `export default {}` — so every session read and write failed in a browser. Added a `localStorage`-backed store selected by platform, keeping the native Keychain path untouched and preserving its ordering guarantee that a refresh token is never retained without its device binding. `localStorage` is explicitly not secure storage, so the web store throws outside a development bundle rather than silently downgrading the guarantee; the web target is not a delivery channel, iOS TestFlight is. The backend must also run with `PERPETO_DEV_CORS_ORIGINS` set, since it otherwise sends no CORS headers.

- Added a development sign-in so Expo Go can reach the authenticated screens (DEC-0014). Expo Go does not ship the native modules `src/auth/providers.ts` needs, so the Apple and Google buttons cannot work there. `AuthController.signInDev` runs the ordinary flow — real challenge, real PKCE verifier, real exchange endpoint, real session — but substitutes a synthetic identity token and never calls `authorizeProvider`, so no native module is touched. It signs in as `GOOGLE`, because the exchange only checks the nonce binding for Apple, and declares the `IOS` platform the backend still requires. Gated behind `EXPO_PUBLIC_DEV_AUTH_ENABLED`, dark by default like the other flags, and it only works against a backend started with `PERPETO_DEV_AUTH=true`, which that backend permits in PAPER mode alone. The auth state machine, session store and screens are unchanged, so the resulting session is indistinguishable from a real one.
