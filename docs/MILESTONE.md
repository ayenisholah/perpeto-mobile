...
| M2 | Scanner and paper trading | Normalized markets, forecasts, opportunity/risk engine, coordinated paper lifecycle and complete mobile monitoring | Feature implementation complete; exact-decimal wire/persistence now verified by a passing PostgreSQL round-trip suite, remaining exit gates are dependent M1 device/accessibility evidence |
...
| M4 | Shared-VPS multi-user CEX pilot | Personal tenants, PostgreSQL RLS, passkeys, Vault Transit credentials, account preflight, tenant risk isolation, and a controlled `PILOT` cohort | In progress (tenant identity, forced-RLS isolation, passkeys, credential lifecycle, preflight and pilot arming are database-verified under a non-`BYPASSRLS` runtime role; Vault infrastructure, device ceremony, limited-capital pilot and signed evidence pending) |
